#include<stdio.h>
#include"closedHashingLinearHashing.h"
...
int main(){
    ...
    // for(int i=0; i<10; i++) deleteFirst(pallete, colors[i]); //0-5
    // for(int i=0; i<10; i++) deleteLast(pallete, colors[i]); //5-7
    // displayDict(pallete);
    ...
    for(int i=0; i<1; i++) displayColor(searchViaString(pallete, colors[i].name));
    printf("================================================\n");
    // printf("\n\n");
